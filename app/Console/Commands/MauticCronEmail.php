...
class MauticCronEmail extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'mauticemail:cron {rule_id}';
            ...
            if($allConnection->type == 'mautic'){ 
              ...
              if(!empty($all_lists)){
                foreach ($all_lists as $key => $all_list) {
                  ...
                  $allEmailLogsArray = EmailLogs::where('rule_number', $rule_id)
                  ->whereIn('type',['webhook','mautic'])
                  ->whereDate('created_at', Carbon::today())->get()->toArray();
                  $allInvalidEmailArray = InvalidEmail::where('rule_number', $rule_id)
                  ->whereIn('type',['webhook','mautic'])
                  ->whereDate('created_at', Carbon::today())->get()->toArray();
                  ...
                  if($checkemail  < $calEmail ){
                    ...
                    if($results->status == 'success'){
                      $emailLogs = array('email' => $value['email'],'status' => 'sucesss','type' => 'webhook_mautic','rule_number' => $value['rule_number'],'rule_name' => $value['rule_name'],'timezone' => $value['timezone']);
                      EmailLogs::create($emailLogs);
                      ...
                      $emailLogs = array('email' => $value['email'],'status' => 'error','type' => 'webhook_mautic','rule_number' => $value['rule_number'],'rule_name' => $value['rule_name'],'timezone' => $value['timezone']);
                      EmailLogs::create($emailLogs);
                      $listemail_status = array('in_pool'=>0);
                      DB::table('listing_email')
                            ->where('email_id',$value['email_id'])
